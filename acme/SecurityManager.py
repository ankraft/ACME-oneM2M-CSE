...
class SecurityManager(object):
	...
	def __init__(self) -> None:
		...
		if L.isInfo: L.log('SecurityManager initialized')
		if self.enableACPChecks:
			if L.isInfo: L.log('ACP checking ENABLED')
		else:
			if L.isInfo: L.log('ACP checking DISABLED')


	def shutdown(self) -> bool:
		if L.isInfo: L.log('SecurityManager shut down')
		return True
	...
	def hasAccess(self, originator:str, resource:Resource, requestedPermission:Permission, checkSelf:bool=False, ty:int=None, isCreateRequest:bool=False, parentResource:Resource=None) -> bool:
		...
		if ty is not None:
			...
			if ty == T.AE and isCreateRequest:
				...
				if originator is None or len(originator) == 0 or Utils.isAllowedOriginator(originator, CSE.registration.allowedAEOriginators):
					if L.isDebug: L.logDebug('Originator for AE CREATE. OK.')
					return True
			...
			if ty == T.CSR and isCreateRequest:
				if Utils.isAllowedOriginator(originator, CSE.registration.allowedCSROriginators):
					if L.isDebug: L.logDebug('Originator for CSR CREATE. OK.')
					return True
				else:
					if L.isWarn: L.logWarn('Originator for CSR CREATE not found.')
					return False

			if T(ty).isAnnounced():
				if Utils.isAllowedOriginator(originator, CSE.registration.allowedCSROriginators) or originator[1:] == parentResource.ri:
					if L.isDebug: L.logDebug('Originator for Announcement. OK.')
					return True
				else:
					if L.isWarn: L.logWarn('Originator for Announcement not found.')
					return False
		...
		if resource.ty == T.CSEBase and requestedPermission & Permission.RETRIEVE:
			...
			if CSE.storage.retrieveResource(aei=originator).resource is not None:
				if L.isDebug: L.logDebug(f'Allow registered AE Orignator {originator} to RETRIEVE CSEBase. OK.')
				return True
			
			# Allow remote CSE to RETRIEVE the CSEBase

			if originator == CSE.remote.registrarCSI:
				if L.isDebug: L.logDebug(f'Allow registrar CSE Originnator {originator} to RETRIEVE CSEBase. OK.')
				return True
			if Utils.isAllowedOriginator(originator, CSE.registration.allowedCSROriginators):
				if L.isDebug: L.logDebug(f'Allow remote CSE Orignator {originator} to RETRIEVE CSEBase. OK.')
				return True
		...
		if resource is None:
			...
			if L.isWarn: L.logWarn('Resource must not be None')
			return False
		if requestedPermission is None or not (0 <= requestedPermission <= Permission.ALL):
			if L.isWarn: L.logWarn('RequestedPermission must not be None, and between 0 and 63')
			return False

		if L.isDebug: L.logDebug(f'Checking permission for originator: {originator}, ri: {resource.ri}, permission: {requestedPermission:d}, selfPrivileges: {checkSelf}')
		...
		if resource.ty == T.GRP: # target is a group resource
			...
			if (macp := resource.macp) is None or len(macp) == 0:
				if L.isDebug: L.logDebug("MembersAccessControlPolicyIDs not provided, using AccessControlPolicyIDs")
				...
				for a in macp:
					if (acp := CSE.dispatcher.retrieveResource(a).resource) is None:
						if L.isDebug: L.logDebug(f'ACP resource not found: {a}')
						continue
					else:
						if acp.checkPermission(originator, requestedPermission, ty):
							if L.isDebug: L.logDebug('Permission granted')
							return True
				if L.isDebug: L.logDebug('Permission NOT granted')
				return False
		...
		if resource.ty in [T.ACP, T.ACPAnnc]:	# target is an ACP or ACPAnnc resource
			...
			if (acpi := resource.acpi) is None or len(acpi) == 0:
				if L.isDebug: L.logDebug('Handle with missing acpi in resource')
				...
				if resource.attributePolicies is not None and 'acpi' in resource.attributePolicies:
					...
					if (holder := resource.hld) is not None:
						if holder == originator:	# resource.holder == originator -> all access
							if L.isDebug: L.logDebug('Allow access for holder')
							return True
					...
					# Check resource creator
						...
						if L.isDebug: L.logDebug('Allow access for creator')
						return True
				...
				# resource doesn't support acpi attribute
					...
					if resource.inheritACP:
						if L.isDebug: L.logDebug('Checking parent\'s permission')
				...
				if L.isDebug: L.logDebug('Permission NOT granted for resource w/o acpi')
				return False
			...
			if targetResource.acpi is None:
				...
				for ri in targetResource.acpi:
					if (acp := CSE.dispatcher.retrieveResource(ri).resource) is None:
						if L.isWarn: L.logWarn(f'Access Check for acpi: referenced <ACP> resource not found: {ri}')
						continue
