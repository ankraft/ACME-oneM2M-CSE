...
from typing import Optional, cast, Type, Tuple, Callable
...
_resourceClassMapping:dict[ResourceTypes, Tuple[Type[Resource], Callable]] = {
...
def getResourceClassForType(ty:ResourceTypes) -> Optional[Type[Resource]]:
	...
	"""
	tup = _resourceClassMapping.get(ty)
	return tup[0] if tup else None


def initResources() -> bool:
	...
	for ty, tup in _resourceClassMapping.items():
		if ty not in resourceTypeDetails:
			raise RuntimeError(f'Unknown resource type: {ty}')
		if not (clazz := getResourceClassForType(ty)):
