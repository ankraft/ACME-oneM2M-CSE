...
from typing import Generator, Optional, Tuple, TextIO, Callable, TYPE_CHECKING
...
class ManagementSupport(metaclass=Singleton):
	"""	Management support class. Provides functions for accessing and managing CSE internal functions.
	...
	def getRegistrations(self) -> str:
		...
		"""
		return json.dumps(self.getRegistrationStatus(), indent=4)
	...
	def getRegistrationStatus(self) -> JSON:
		...
		# CSE registrations
		for csr in self.dispatcher.retrieveResourcesByType(ResourceTypes.CSR):
			_e = {
				'CSE-ID': csr.csi,
		...
		for ae in self.dispatcher.retrieveResourcesByType(ResourceTypes.AE):
			status['ae'].append({
				'AE-ID': ae.aei,
				'resourceID': ae.ri,
				'resourceName': ae.rn,
	...
	def getRequests(self) -> Generator[str, None, None]:
		...
		def generate() -> Generator[str, None, None]:
			_rb = self.requestManager.requestRingBuffer
...
"""
		...
		for r in self.storage.getRequests(id, sortedByOt=True):
			req = r['req']
			op = req['op']
			to = None
	...
	#########################################################################
	#
	#	General status related functions
	#	
	...
	def getCSEStatus(self) -> str:
		...
		"""
		if (d := self.getCSEStatusAsDict()) is None:
	...
	def getCSEStatusAsDict(self) -> JSON:
		...
		try:
			...
			status['resources'].update({
				'AE': (_cAE := self.dispatcher.countResources(ResourceTypes.AE)),
				'ACP':(_cACP := self.dispatcher.countResources(ResourceTypes.ACP)),
				'ACTR': (_cACTR := self.dispatcher.countResources(ResourceTypes.ACTR)),
				'ALST': (_cALST := self.dispatcher.countResources(ResourceTypes.ALST)),
				'CB': (_cCB := self.dispatcher.countResources(ResourceTypes.CSEBase)),
				'CIN': (_cCIN := self.dispatcher.countResources(ResourceTypes.CIN)),
				'CNT': (_cCNT := self.dispatcher.countResources(ResourceTypes.CNT)),
				'CRS': (_cCRS := self.dispatcher.countResources(ResourceTypes.CRS)),
				'CSR': (_cCSR := self.dispatcher.countResources(ResourceTypes.CSR)),
				'DEPR': (_cDEPR := self.dispatcher.countResources(ResourceTypes.DEPR)),
				'FCNT': (_cFCNT := self.dispatcher.countResources(ResourceTypes.FCNT)),
				'FCI': (_cFCI := self.dispatcher.countResources(ResourceTypes.FCI)),
				'GRP': (_cGRP := self.dispatcher.countResources(ResourceTypes.GRP)),
				'LCP': (_cLCP := self.dispatcher.countResources(ResourceTypes.LCP)),
				'MGMTOBJ': (_cMGMTOBJ := self.dispatcher.countResources(ResourceTypes.MGMTOBJ)),
				'NOD': (_cNOD := self.dispatcher.countResources(ResourceTypes.NOD)),
				'NTP': (_cNTP := self.dispatcher.countResources(ResourceTypes.NTP)),
				'NTPR': (_cNTPR := self.dispatcher.countResources(ResourceTypes.NTPR)),
				'PCH': (_cPCH := self.dispatcher.countResources(ResourceTypes.PCH)),
				'PDR': (_cPDR := self.dispatcher.countResources(ResourceTypes.PDR)),
				'REQ': (_cREQ := self.dispatcher.countResources(ResourceTypes.REQ)),
	...
	def getStatusRich(self,
		...
		if (status := self.getCSEStatusAsDict()) is None:
	...
	def getStructurePuml(self, maxLevel: Optional[int] = 0) -> str:
		...
		def getChildren(res:Resource, level:int) -> str:
			...
			if maxLevel > 0 and level == maxLevel:
				return result
			chs = self.dispatcher.retrieveDirectChildResources(res.ri)
			for ch in chs:
...
"""
	...
	def getResourceTreeText(self, maxLevel: int = 0) -> str:
		...
		console.print(self.getResourceTreeRich(maxLevel=maxLevel, withProgress=False))
	...
	def getResourceTreeRich(self, 
		...
		def info(res: Resource) -> str:
			...
			if treeMode in [ TreeMode.CONTENT, TreeMode.CONTENTONLY ]:
				match res.ty:
					...
					case ResourceTypes.FCNT | ResourceTypes.FCI:
						contentInfo = ', '.join([ f'{attr}={str(res[attr])}' for attr in res.dict if self.validator.isExtraResourceAttribute(attr, res) ])
		...
		def getTree() -> Optional[Tree]:
			...
			"""
			if parent:
				if not (res := self.dispatcher.retrieveResource(parent)):
					return None
			else:
				res = getCSE()
			if not res:
			...
			return json.dumps(
				{ 	'unloadedPlugins': sorted(pluginManager.unloadedPlugins, key=lambda p: p.lower()),
					...
					'loadedPlugins': sorted([ {
						...
						'filename': p.fileName,
						'dependencies': [{	('module' if not pluginManager.isProvidedFunction(d.pluginName) else 'function'): d.pluginName, 
					...
					'providedDependencies': sorted([ {
						('class' if not moduleName[1] else 'function'): moduleName[0],
						'providedTo': sorted([
								{	'class': f'{mod}.{cls}', 
									'attribute': attr
								} 
								for mod, cls, attr in instances
							], key=lambda d: d['class'].lower())	# type: ignore[index]
						}		
						for moduleName, instances in dgp.items()
					], 
					key=lambda d: d['class'].lower() if 'class' in d else d['function'].lower())
	...
	def getServices(self) -> str:
			...
			return json.dumps(
				{ 	name: {
					...
					if name != 'acmecse.runtime.PluginSupport.ServicePlugin'	# Ignore a helper class that is not a real service
				}, indent=4)
	...
	def doExportResource(self, ri: str, withChildResources: Optional[bool] = False) -> Tuple[int, str]:
		...
		try:
			...
			if withChildResources:
				resdis = self.dispatcher.discoverResources(ri, originator=RC.cseOriginator)
				# insert the parent resource at the beginning of the list
				resdis.insert(0, self.dispatcher.retrieveResource(ri))
			else:
				resdis = [self.dispatcher.retrieveResource(ri)]

			# Counter for the number of resources exported
			count = 0

			# Create a temporary directory for the export
			outdir = f'{Configuration.baseDirectory}/tmp'
...
''')
				...
				for r in resdis:
					...
					for attr in r.getAttributes():
						policy = self.validator.getAttributePolicy(r.ty, attr)
	...
	def doExportInstances(self, ri: str, asString: Optional[bool] = False) -> Tuple[int, str]:
		...
		try:
			...
			container = self.dispatcher.retrieveResource(ri)
			...
			if not (instances := self.dispatcher.retrieveDirectChildResources(ri, _instanceMapping[container.ty])):
	...
	def resetCSE(self) -> None:
		"""Reset the CSE to its initial state.
		"""
		self.cseReset()


	def restartCSE(self) -> None:
		"""Restart the CSE.
